...
public class ChatServer implements ServerSocketThreadListener, SocketThreadListener {
    ...
    private static final Logger commonLogger = Logger.getLogger("");

    public ChatServer(ChatServerListener listener) throws IOException {
        this.listener = listener;
        clients = new Vector<>();
        logger.setLevel(Level.FINE);
        Handler handler = new FileHandler("log_%g.txt", 1024 * 10, 10,true);
...
//        putLog("Server socket thread accept timeout");
    ...
    public void onSocketAccepted(ServerSocketThread thread, ServerSocket server, Socket socket) {
        ...
        logger.log(Level.INFO,"Клиент " + name + " подключился");
    }
    ...
    public synchronized void onReceiveString(SocketThread thread, Socket socket, String msg) {
        ...
        ClientThread client = (ClientThread) thread;
        logger.log(Level.INFO,"Клиент " + client.getName()+ " Прислал сообщение/комманду");
    ...
    private void handleAuthClientMessage(ClientThread client, String msg) {
        ...
        switch (msgType) {
            ...
            case Protocol.USER_CHANGE_NICKNAME:
                try {
                    if (sqlClient.changeNickname(arr[1], client.getNickname()) > 0) {
